...
public class CrossClustersQueryIT extends AbstractMultiClustersTestCase {
    ...
    public void testSearchesWhereMissingIndicesAreSpecified() {
        ...
        {
            String query = "FROM logs-*,cluster-a:nomatch | STATS sum (v)";
            if (skipUnavailable == false) {
                // throws an exception when skip_unavailable=false and concrete index was specified
                IndexNotFoundException exception = expectThrows(IndexNotFoundException.class, () -> runQuery(query, requestIncludeMeta));
                assertThat(exception.getDetailedMessage(), containsString("nomatch"));
            } else {
                ...
                try (EsqlQueryResponse resp = runQuery(query, requestIncludeMeta)) {
                    ...
                    assertThat(remoteCluster.getFailures().get(0).reason(), containsString("no such index [nomatch]"));
        ...
        try (
            EsqlQueryResponse resp = runQuery(
                "from nomatch,*:logs-* | stats count(*) by tag | sort tag | keep tag",
                requestIncludeMeta
            )
        ) {
            List<List<Object>> values = getValuesList(resp);
            ...
            assertThat(localCluster.getFailures().get(0).reason(), containsString("no such index [nomatch]"));
        }

        {
            String query = "FROM no_such_index*,*:no_such_index1,*:no_such_index2,logs-1 | STATS COUNT(*) by tag | SORT tag | KEEP tag";
            if (skipUnavailable == false) {
                // throws an exception when skip_unavailable=false and concrete index was specified
                IndexNotFoundException exception = expectThrows(IndexNotFoundException.class, () -> runQuery(query, requestIncludeMeta));
                assertThat(exception.getDetailedMessage(), containsString("no such index [no_such_index1,no_such_index2]"));
            } else {
                ...
                try (EsqlQueryResponse resp = runQuery(query, requestIncludeMeta)) {
                    ...
                    assertThat(remoteCluster.getFailures().size(), equalTo(1));
                    assertThat(
                        remoteCluster.getFailures().get(0).reason(),
                        containsString("no such index [no_such_index1,no_such_index2]")
                    );
        ...
        // MP TODO: edit the comments below - I combined two from the merge conflict
        ...
        // local index matches, wildcard on remote cluster that matches nothing - should be present in EsqlExecutionInfo marked as
        // SKIPPED, no shards searched, with failure message
        try (EsqlQueryResponse resp = runQuery("from cluster-a:no_such_index*,logs-* | stats sum (v)", requestIncludeMeta)) {
            ...
            assertThat(remoteCluster.getFailedShards(), equalTo(0));
            assertThat(remoteCluster.getFailures().size(), equalTo(1));
            assertThat(remoteCluster.getFailures().get(0).reason(), containsString("no such index [no_such_index*]"));
